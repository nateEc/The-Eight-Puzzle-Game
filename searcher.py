...
class BFSearcher(Searcher):
    """ a searcher class that perform breadth-first search (BFS) 
...
class DFSearcher(Searcher):
    """ searcher objects that perform depth-first search (DFS) 
    instead of random search.
    """
    def next_state(self):
        s = self.states[-1]
...
def h1(state):
    ...
    and returns an estimate of how many additional moves are 
    needed to get from state to the goal state. 
    """
    return state.board.num_misplaced()

def h2(state):
    return (state.board.num_false_row() + state.board.num_false_col())
...
class GreedySearcher(Searcher):
    ...
    def add_state(self, state):
        ...
        inherited from Searcher. Rather than simply adding the specified
        state to the list of untested states, the method should add a 
        sublist that is a [priority, state] pair, where priority is the
...
class AStarSearcher(GreedySearcher):
    ...
    each state based on a heuristic function, and that selects the next
    state based on those priorities. 
    """
    def priority(self, state):
        """ However, when A* assigns a priority to a state, it also take
        into account the cost that has already been expended to get to that
